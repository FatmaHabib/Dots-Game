...
int N, POSITION_X = 0, POSITION_Y = 0, turn = 0, ScorePlayer1 = 0, ScorePlayer2 = 0, ScorePlayer3 = 0, ScorePlayer4 = 0, module1 = 0, module2 = 0;
string SCORE;
...
void DRAWBOARD(int N)
{
	...
	if (ScorePlayer1>9)
	{
		ScorePlayer4 = ScorePlayer1 / 10;
		SCORE += (ScorePlayer4 + '0');
		module1 = ScorePlayer1 % 10;
		SCORE += (module1 + '0');
	}
	else
	...
	if (ScorePlayer2>9)
	{
		ScorePlayer3 = ScorePlayer2 / 10;
		SCORE += (ScorePlayer3 + '0');
		module2 = ScorePlayer2 % 10;
		SCORE += (module2 + '0');
	}
	else
		...
		while (window.pollEvent(event))
		{
			if (event.type == Event::MouseButtonPressed)
			{
				if (event.mouseButton.button == Mouse::Left)
				{
					...
					{
						if (POSITION_X >= 26 && POSITION_X <= 279 && POSITION_Y >= 49 && POSITION_Y <= 87)
						{
							WIN = false;
							AllLines.clear();
							square.clear();
						...
						else if (POSITION_X >= 678 && POSITION_X <= 797 && POSITION_Y >= 39 && POSITION_Y <= 90)
						{
							window.close();
						}
					}
					...
					{
						if (POSITION_X >= 41 && POSITION_X <= 297 && POSITION_Y >= 40 && POSITION_Y <= 83)
						{
							WIN = false;
							AllLines.clear();
							square.clear();
						...
						else if (POSITION_X >= 696 && POSITION_X <= 790 && POSITION_Y >= 33 && POSITION_Y <= 86)
						{
							window.close();
						}
					}
					...
					{
						if (POSITION_X >= 13 && POSITION_X <= 299 && POSITION_Y >= 491 && POSITION_Y <= 524)
						{
							WIN = false;
							AllLines.clear();
							square.clear();
						...
						else if (POSITION_X >= 13 && POSITION_X <= 161 && POSITION_Y >= 532 && POSITION_Y <= 557)
						{
							window.close();
						}
					}
		...
		while (window.pollEvent(event))
		{
			...
			if (event.type == sf::Event::KeyPressed)
			{
				...
				if (event.key.code == 58)
					...
					continue;
				else if (event.key.code == 33 || event.key.code == 34 || event.key.code == 35)
					continue;
				else if (event.key.code == 59 && m != "")
					m.pop_back();
				number = true;
